...
plot <- ggplot(immune_cells_boxplot_df, aes(x = Condition, y = GSVA_score, fill = Condition)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7, width = 0.35) +
  stat_compare_means(
    method = "wilcox.test", 
    label = "p.format", 
    size = 1.5, 
...
CorLevelPlot(heatmap.data,
             ...
             y = names(heatmap.data)[1:module_eigengenes_length],
             col = c("green", "lightgreen", "white", "pink", "red"))
dev.copy(png, filename = "wgcna/FRDA_All_Batches_ENSG_genes_removed/wgcna_module_trait_relationships_all_batch.png", width = 2000, height = 1500, res = 150)
dev.off()
...
boxplot_df <- bwnet$MEs |>
  ...
  pivot_longer(cols = all_of(colnames(bwnet$MEs)),
              names_to = "Module",
              values_to = "Eigengene") |> 
  filter(Module %in% rownames(module_trait_cor)) |> 
...
plot <- ggplot(boxplot_df, aes(x = Condition, y = Eigengene, fill = Condition)) +
  ...
  theme(
    ...
    # legend.box.margin = margin(15, 20, 15, 20),  # top, right, bottom, left
    legend.background = element_blank())
dev.copy(png, filename = "wgcna/FRDA_All_Batches_ENSG_genes_removed/boxplot_all_modules.png", width = 2000, height = 1500, res = 150)
dev.off()
