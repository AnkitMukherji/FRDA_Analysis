...
# Correlation between DD*GAA1 and severity groups
# Expected: Severe > Moderate > Mild for DD*GAA1

# Compute DD*GAA1 for patients
corr_data <- frda_patients |>
  rownames_to_column("SampleID") |>
  filter(severity_group %in% c("Mild", "Moderate", "Severe")) |>
  mutate(
...
pw_df <- as.data.frame(pairwise_wilcox$p.value) |>
  ...
  filter(!is.na(p))
y_max <- max(corr_data$DD_x_GAA1, na.rm = TRUE)
...
# Boxplot: DD*GAA1 by severity group
p_dd_gaa1 <- ggplot(corr_data, aes(x = severity_group, y = DD_x_GAA1, fill = severity_group)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7) +
  ...
  theme_bw(base_size = 14) +
  labs(
    title = "DD × GAA1 by Severity Group",
...
# Add wilcox p-values to each group comparison
p_dd_gaa1 <- p_dd_gaa1 +
  stat_pvalue_manual(
    pw_df,
    label = "p.label",
...
ggsave(
  "patient_stratification_prediction/patient_stratify/DD_GAA1_by_severity.png",
  plot = p_dd_gaa1,
  device = "png",
  units = "in",
  height = 6,
...
ggsave(
  "patient_stratification_prediction/patient_stratify/DD_vs_GAA1_scatter.png",
  plot = p_scatter,
  device = "png",
  units = "in",
  height = 6,
...
severity_corr_data <- frda_patients |>
  as.data.frame() |>
...
ggsave(
  "patient_stratification_prediction/patient_stratify/severity_scales_correlation.png",
