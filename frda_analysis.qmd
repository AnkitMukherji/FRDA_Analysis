...
# Linear regression for each gene with each trait
results_list <- list()

for (trait in traits_of_interest) {

    clean_samples <- complete.cases(meta[, c(trait, "Age", "Sex", "Batch")])

    meta_sub <- meta[clean_samples, ]
    expr_sub <- expr_vst[, clean_samples]

    meta_sub$Batch <- droplevels(factor(meta_sub$Batch))
    meta_sub$Sex   <- droplevels(factor(meta_sub$Sex))

    design <- model.matrix(
        as.formula(paste0("~ `", trait, "` + Age + Sex + Batch")),
        data = meta_sub
    )

    if (qr(design)$rank < ncol(design)) {
        message("Design matrix not full rank for trait: ", trait)
        next
    }

    aw  <- arrayWeights(expr_sub, design)
    fit <- lmFit(expr_sub, design, weights = aw)
    fit <- eBayes(fit, robust = TRUE)

    res <- topTable(fit, coef = trait, number = Inf) |> 
      rownames_to_column("gene_names") |> 
      as_tibble()
    results_list[[trait]] <- res
}
...
write_xlsx(top_table, "patient_stratification_prediction/patient_stratify/top_genes_from_regression.xlsx")
...
plot_trends <- function(trait) {
    ...
    plot_data_list <- list()
    for (gene in top_table$gene_names) {
...
plot_volcano_cor <- function(trait_name) {
  ...
  ggplot(df, aes(x = logFC, y = negLogP)) +
    ...
    geom_text_repel(data = top_genes, aes(label = gene_names), size = 3, max.overlaps = 20) +
