...
dds_frda_all <- readRDS("dds_frda_all.rds")
vst_frda_all <- vst(dds_frda_all, blind = FALSE)

# Pathway enrichment analysis
dge_frda_all <- read_xlsx("res/res_all_frda_patients_vs_controls.xlsx")
...
# order the samples to cluster patients and controls together
order_index <- order(vst_frda_all$Condition == "Patient", decreasing = TRUE)
vst_frda_all <- vst_frda_all[, order_index]
...
plot(sample_tree, main = "Sample clustering to detect outliers", sub="", xlab="", cex.lab = 1.5, 
     cex.axis = 1.5, cex.main = 2)
dev.copy(png, filename = "wgcna/FRDA_All_Batches/wgcna_sample_clustering_all_batch.png", width = 3000, height = 1500, res = 150)
dev.off()

# Remove outlier sample FA102
outlier_sample <- "FA102"
expr_rb_wgcna_filtered <- expr_rb_wgcna_filtered[, colnames(expr_rb_wgcna_filtered) != outlier_sample]
metadata_frda_wgcna <- metadata_frda_all[rownames(metadata_frda_all) != outlier_sample, ]

# Recluster samples after removing outlier
sample_tree <- hclust(dist(t(expr_rb_wgcna_filtered)), method = "average")
plot(sample_tree, main = "Sample clustering after outlier removal", sub="", xlab="", cex.lab = 1.5, 
     cex.axis = 1.5, cex.main = 2)
dev.copy(png, filename = "wgcna/FRDA_All_Batches/wgcna_sample_clustering_all_batch_outlier_removed.png", width = 3000, height = 1500, res = 150)
dev.off()
...
grid.arrange(a1, a2, nrow = 2)
dev.copy(png, filename = "wgcna/FRDA_All_Batches_gene_filtered/wgcna_soft_thresholding_power_selection_all_batch.png", width = 2000, height = 2500, res = 150)
dev.off()
...
bwnet <- blockwiseModules(norm_count_data,
                 ...
                 numericLabels = FALSE,
                 saveTOMs = TRUE,
                 saveTOMFileBase = "wgcna/FRDA_All_Batches_gene_filtered/frda_all_batch_wgcna_TOM",
                 randomSeed = 1234,
                 verbose = 3)

saveRDS(bwnet, file = "wgcna/FRDA_All_Batches_gene_filtered/wgcna_frda_all_batch.rds")
bwnet <- readRDS("wgcna/FRDA_All_Batches_gene_filtered/wgcna_frda_all_batch.rds")
...
plotDendroAndColors(bwnet$dendrograms[[1]], cbind(bwnet$unmergedColors, bwnet$colors),
                    ...
                    hang= 0.03,
                    guideHang = 0.05)
dev.copy(png, filename = "wgcna/FRDA_All_Batches_gene_filtered/wgcna_gene_dendrogram_all_batch.png", width = 2000, height = 1500, res = 150)
dev.off()

# Relate modules to external traits
metadata_frda_wgcna$condition_categorize <- ifelse(metadata_frda_wgcna$Condition == "Control", 0, 1)
metadata_frda_wgcna$sex_categorize <- ifelse(metadata_frda_wgcna$Sex == "F", 0, 1)
...
CorLevelPlot(heatmap.data,
             ...
             y = names(heatmap.data)[1:module_eigengenes_length],
             col = c("green", "lightgreen", "white", "pink", "red"))
dev.copy(png, filename = "wgcna/FRDA_All_Batches_gene_filtered/wgcna_module_trait_relationships_all_batch.png", width = 2000, height = 1500, res = 150)
dev.off()
...
write_xlsx(output_list, "wgcna/FRDA_All_Batches_gene_filtered/genes_in_significant_modules_all_batch.xlsx")
...
for (mod in significant_modules) {
  ...
  ggsave(paste0("wgcna/FRDA_All_Batches_gene_filtered/GO_enrichment_module_", mod, "_all_batch.png"),
         plot = p,
         width = 10,
         height = 6,
         dpi = 300)
...
write.xlsx(GO_enrichment_results, "wgcna/FRDA_All_Batches_gene_filtered/go_enrichment_significant_modules_all_batch.xlsx")
...
if (websiteLive) {
  ...
  for (db in names(enrichr_enrich)) {
    ...
    ggsave(paste0("wgcna/FRDA_All_Batches_gene_filtered/enrichr_", gsub(" ", "_", tolower(db)), "_module_", mod, "_all_batch.png"),
           plot = p,
           width = 10,
           height = 6,
           dpi = 300)
...
clean_name <- function(x) substr(x, 1, 31)
names(enrichr_results) <- sapply(names(enrichr_results), clean_name)
write.xlsx(enrichr_results, "wgcna/FRDA_All_Batches_gene_filtered/enrichr_significant_modules_all_batch.xlsx")
