...
# Differential expression analysis between patients with and without HCM
metadata_frda_all <- colData(dds_frda_all) |> as.data.frame()
count_data_frda_all <- assay(dds_frda_all) |> as.data.frame()

metadata_frda_all$HCM[is.na(metadata_frda_all$HCM)] <- 0

all(rownames(metadata_frda_all) == colnames(count_data_frda_all))
dge_frda_hcm_non_hcm <- dge(counts = count_data_frda_all,
                    meta = metadata_frda_all,
                    design = ~ HCM + Batch + Sex + Age,
                    filename = "res/res_all_frda_ensg_removed_hcm_non_hcm.xlsx",
                    filename_volcano = "volcano_plot/volcano_all_frda_ensg_removed_hcm_non_hcm.jpeg",
                    lfc_threshold = 1,
                    padj_threshold = 0.05,
                    top_n = 10)

metadata_frda_all_patients <- metadata_frda_all[metadata_frda_all$Condition == "Patient", ]
metadata_frda_all_patients$Condition <- droplevels(metadata_frda_all_patients$Condition)
count_data_frda_all_patients <- count_data_frda_all[, rownames(metadata_frda_all_patients)]

all(rownames(metadata_frda_all_patients) == colnames(count_data_frda_all_patients))
dge_frda_patients_hcm_non_hcm <- dge(counts = count_data_frda_all_patients,
                    meta = metadata_frda_all_patients,
                    design = ~ HCM + Batch + Sex + Age,
                    filename = "res/res_all_frda_patients_ensg_removed_hcm_non_hcm.xlsx",
                    filename_volcano = "volcano_plot/volcano_all_frda_patients_ensg_removed_hcm_non_hcm.jpeg",
                    lfc_threshold = 1,
                    padj_threshold = 0.05,
                    top_n = 10)
...
col_to_fill_zero <- c("HCM", "DD", "DD x GAA1", "FSA scale", "mFARS total", "mFARS USS")
metadata_frda_wgcna[col_to_fill_zero] <- lapply(metadata_frda_wgcna[col_to_fill_zero], function(x) {
  x[is.na(x)] <- 0
  return(x)
})

col_to_num_convert <- c("HCM", "DD", "DD x GAA1", "FSA scale", "mFARS total", "mFARS USS")
...
trait_data <- metadata_frda_wgcna |> 
  select(Condition_0_1, HCM, GAA1, GAA2, DD, `DD x GAA1`, `FSA scale`, `mFARS total`, `mFARS USS`)
...
```{r, eval=FALSE}
dds_frda_all <- readRDS("dds_frda_all_ensg_removed.rds")
metadata_frda_all <- as.data.frame(colData(dds_frda_all))
count_data_frda_all <- as.data.frame(assay(dds_frda_all))
# Filtering FRDA patients only
frda_patients <- metadata_frda_all[metadata_frda_all$Condition == "Patient", ]

# Removing patients with heterozygous repeat length and rerun everything
frda_patients_hom <- metadata_frda_all[metadata_frda_all$GAA1 > unique(metadata_frda_all$GAA1[metadata_frda_all$Condition == "Control"]), ]

# Converting severity scales to numeric
frda_patients_hom$FSA_scale_num <- as.numeric(frda_patients_hom$FSA.scale)
frda_patients_hom$mFARS_total_num <- as.numeric(frda_patients_hom$mFARS.total)
frda_patients_hom$mFARS_USS_num <- as.numeric(frda_patients_hom$mFARS.USS)

# Total patients with data for each scale
n_fsa <- sum(!is.na(frda_patients_hom$FSA_scale_num))
n_mfars_total <- sum(!is.na(frda_patients_hom$mFARS_total_num))
n_mfars_uss <- sum(!is.na(frda_patients_hom$mFARS_USS_num))

# Histogram for FSA scale
p1 <- ggplot(frda_patients_hom, aes(x = FSA_scale_num)) +
...
p2 <- ggplot(frda_patients_hom, aes(x = mFARS_total_num)) +
...
p3 <- ggplot(frda_patients_hom, aes(x = mFARS_USS_num)) +
...
# Save combined plot
ggsave(
  "patient_stratification_prediction/patient_stratify/severity_scales_histograms_hom_patients.png",
  plot = combined_hist,
  device = "png",
  units = "in",
  height = 12,
...
severity_df <- frda_patients_hom |>
  transmute(
    SampleID = rownames(frda_patients_hom),
    FSA = FSA_scale_num,
...
# Joining results back to frda_patients_hom
frda_patients_hom <- frda_patients_hom |>
  mutate(SampleID = rownames(frda_patients_hom)) |>
  left_join(
...
# Counts per group
print(table(frda_patients_hom$severity_group, useNA = "ifany"))

# Counts among patients included in grouping (Mild/Moderate/Severe)
sev <- frda_patients_hom$severity_group[
  frda_patients_hom$severity_group %in% c("Mild", "Moderate", "Severe")
]

group_n_named <- table(sev)
...
p_index <- ggplot(
  frda_patients_hom |> filter(severity_group %in% c("Mild", "Moderate", "Severe")),
...
ggsave(
  "patient_stratification_prediction/patient_stratify/severity_index_histogram_hom_patients.png",
  plot = p_index,
  device = "png",
  units = "in",
  height = 8,
...
ggsave(
  "patient_stratification_prediction/patient_stratify/severity_scales_boxplots_hom_patients.png",
  plot = p_box,
  device = "png",
  units = "in",
  height = 12,
...
# Correlation between DD, GAA1 and DD*GAA1 and severity groups
# Expected: Severe > Moderate > Mild for DD*GAA1
corr_data <- frda_patients_hom |>
  rownames_to_column("SampleID") |>
  filter(severity_group %in% c("Mild", "Moderate", "Severe")) |>
  mutate(
    DD_num = as.numeric(DD),
    GAA1_num = as.numeric(GAA1),
    DD_x_GAA1 = DD_num * GAA1_num,
    severity_group = factor(severity_group, levels = c("Mild", "Moderate", "Severe"))
  ) |>
  select(SampleID, severity_group, DD_num, GAA1_num, DD_x_GAA1) |>
  filter(!is.na(severity_group))

analyze_metric_by_severity <- function(data, value_col, y_label, title_text, output_file) {
  analysis_data <- data |>
    transmute(SampleID, severity_group, value = {{ value_col }}) |>
    filter(!is.na(value))

  # Statistical test: Kruskal-Wallis (non-parametric, handles non-normal distributions)
  kruskal_test <- kruskal.test(value ~ severity_group, data = analysis_data)
  print(kruskal_test)

  # Pairwise comparisons (Wilcoxon rank-sum)
  pairwise_wilcox <- pairwise.wilcox.test(
    analysis_data$value,
    analysis_data$severity_group,
    p.adjust.method = "BH"
  )
  print(pairwise_wilcox)

  pw_df <- as.data.frame(pairwise_wilcox$p.value) |>
    tibble::rownames_to_column("group1") |>
    pivot_longer(-group1, names_to = "group2", values_to = "p") |>
    filter(!is.na(p))
  y_max <- max(analysis_data$value, na.rm = TRUE)

  if (nrow(pw_df) > 0) {
    pw_df <- pw_df |>
      mutate(
        y.position = seq(y_max * 1.10, y_max * 1.25, length.out = n()),
        p.label = paste0("p = ", signif(p, 3))
      )
  }

  p <- ggplot(analysis_data, aes(x = severity_group, y = value, fill = severity_group)) +
    geom_boxplot(outlier.shape = NA, alpha = 0.7) +
    geom_jitter(width = 0.15, size = 1.5, alpha = 0.6, color = "black") +
    scale_fill_manual(values = c(Mild = "#7ecc51", Moderate = "#F1A340", Severe = "#C93A5A")) +
    theme_bw(base_size = 14) +
    labs(
      title = title_text,
      subtitle = paste0("Kruskal-Wallis p = ", format(kruskal_test$p.value, digits = 3)),
      x = "Severity Group",
      y = y_label,
      fill = "Group"
    ) +
  ...
  invisible(list(kruskal_test = kruskal_test, pairwise_wilcox = pairwise_wilcox, plot = p))
}

res_dd_gaa1 <- analyze_metric_by_severity(
  corr_data,
  DD_x_GAA1,
  "DD x GAA1",
  "DD x GAA1 by Severity Group",
  "patient_stratification_prediction/patient_stratify/DD_GAA1_by_severity_hom_patients.png"
)

res_gaa1 <- analyze_metric_by_severity(
  corr_data,
  GAA1_num,
  "GAA1",
  "GAA1 by Severity Group",
  "patient_stratification_prediction/patient_stratify/GAA1_by_severity_hom_patients.png"
)

res_dd <- analyze_metric_by_severity(
  corr_data,
  DD_num,
  "Disease Duration (years)",
  "Disease Duration by Severity Group",
  "patient_stratification_prediction/patient_stratify/DD_by_severity_hom_patients.png"
)

# Scatter plot: DD vs GAA1 colored by severity
p_scatter <- ggplot(corr_data, aes(x = GAA1_num, y = DD_num, color = severity_group)) +
  geom_point(size = 2.5, alpha = 0.7) +
  scale_color_manual(values = c(Mild = "#7ecc51", Moderate = "#F1A340", Severe = "#C93A5A")) +
  theme_bw(base_size = 14) +
  labs(
    title = "Disease Duration vs GAA1 Repeat Length",
    subtitle = "Colored by severity group",
    x = "GAA1 Repeat Length",
...
ggsave(
  "patient_stratification_prediction/patient_stratify/DD_vs_GAA1_scatter_hom_patients.png",
  plot = p_scatter,
  device = "png",
  units = "in",
  height = 6,
...
severity_patients <- frda_patients_hom |>
  rownames_to_column("SampleID") |>
...
writexl::write_xlsx(
  roc_results,
  "patient_stratification_prediction/patient_stratify/gene_level_ROC_control_vs_states_hom_patients.xlsx"
)
...
run_state_vs_control_dge <- function(state_label) {
  ...
  dge(counts = count_sub,
      ...
      filename = paste0("res/patient_state/res_", state_label, "_vs_Control_hom_patient s.xlsx"),
      filename_volcano = paste0("volcano_plot/patient_state/volcano_", state_label, "_vs_Control_hom_patients.jpeg"),
      lfc_threshold = 1,
      padj_threshold = 0.05,
      top_n = 10)
}
...
# Correlation between severity scales (FSA, mFARS total, mFARS USS)
# Using Spearman correlation because clinical scales are often non-linear and ordinal in nature
severity_corr_data <- frda_patients_hom |>
  as.data.frame() |>
  select(FSA_scale_num, mFARS_total_num, mFARS_USS_num) |>
  drop_na()

# Helper function for scatter plots with correlation
plot_severity_cor <- function(data, x, y, xlabel, ylabel) {
  ggscatter(data, x = x, y = y,
            ...
            color = "#2E4053", shape = 21, size = 3, fill = "#AED6F1", alpha = 0.7) +
    theme_bw(base_size = 14) +
    theme(plot.title = element_text(hjust = 0.5, face = "bold"))
}

p_fsa_mfars <- plot_severity_cor(severity_corr_data, "mFARS_total_num", "FSA_scale_num", "mFARS Total", "FSA Scale")
p_fsa_uss   <- plot_severity_cor(severity_corr_data, "mFARS_USS_num", "FSA_scale_num", "mFARS USS", "FSA Scale")
p_mfars_uss <- plot_severity_cor(severity_corr_data, "mFARS_total_num", "mFARS_USS_num", "mFARS Total", "mFARS USS")

# Combine plots using patchwork
combined_severity_cor <- (p_fsa_mfars | p_fsa_uss | p_mfars_uss) +
  plot_annotation(title = "Correlation between FRDA Severity Scales",
                  theme = theme(plot.title = element_text(size = 18, face = "bold", hjust = 0.5)))

ggsave(
  "patient_stratification_prediction/patient_stratify/severity_scales_correlation_hom_patients.png",
  plot = combined_severity_cor,
  device = "png",
  units = "in",
  height = 6,
  width = 18,
...
# Level of FXN among patients in mild, moderate and severe
fxn_severity_data <- data.frame(
  SampleID = colnames(expr_rb_all),
  FXN_expression = as.numeric(expr_rb_all["FXN", ])
) |>
  inner_join(
    frda_patients_hom |> rownames_to_column("SampleID") |> select(SampleID, severity_group),
    by = "SampleID"
  ) |>
  filter(severity_group %in% c("Mild", "Moderate", "Severe"))

fxn_plot_data <- fxn_severity_data |>
  arrange(severity_group, FXN_expression) |>
  mutate(SampleID = factor(SampleID, levels = SampleID))

p_fxn_indiv <- ggplot(fxn_plot_data, aes(x = SampleID, y = FXN_expression, fill = severity_group)) +
  geom_col(alpha = 0.8) +
  facet_wrap(~severity_group, scales = "free_x", ncol = 3) +
  scale_fill_manual(values = c(Mild = "#7ecc51", Moderate = "#F1A340", Severe = "#C93A5A")) +
  ...
  labs(
    title = "Individual Patient FXN Expression by Severity",
    x = "Patients (Sorted by Expression)",
...
ggsave(
  "patient_stratification_prediction/patient_stratify/FXN_individual_by_severity_hom_patients.png",
  plot = p_fxn_indiv,
  device = "png",
  units = "in",
  height = 6,
...
# Stratifying patients into mild, moderate and severe using a probabilistic model

## Extract Latent Clinical Severity

# Filter for patients with complete clinical scores
# Based on metadata_frda preparation earlier: "FSA scale", "mFARS total", "mFARS USS"
clinical_vars <- c("FSA_scale_num", "mFARS_total_num", "mFARS_USS_num")
patients_all_scores <- frda_patients_hom |> 
  filter(!is.na(FSA_scale_num) & !is.na(mFARS_total_num) & !is.na(mFARS_USS_num))

# Perform Factor Analysis to derive a latent severity score
# This models shared disease burden rather than averaging
fa_res <- fa(patients_all_scores[, clinical_vars], nfactors = 1, fm = "minres", scores = "regression")
patients_all_scores$Latent_Severity <- as.numeric(fa_res$scores)

# Visualize Latent Severity vs mFARS Total
p <- ggplot(patients_all_scores, aes(x = Latent_Severity, y = mFARS.total)) +
  geom_point() +
  geom_smooth(method = "lm", color = "red") +
  theme_bw() +
  labs(title = "Latent Severity (FA) vs mFARS Total", 
       subtitle = "Modeling latent disease burden",
       x = "Latent Severity Score", y = "mFARS Total")

## Identify Natural Severity States (GMM)

# Apply Gaussian Mixture Modeling on the latent severity axis
# BIC determines the optimal number of biological states
gmm_res <- Mclust(patients_complete$Latent_Severity, G = 1:5)
summary(gmm_res)

# Plot BIC for model selection justification
plot(gmm_res, what = "BIC")

# Assign clusters and relabel them as Mild, Moderate, Severe
patients_complete$GMM_Cluster <- factor(gmm_res$classification)

# Ensure alphabetical/factor order matches severity (Mild < Moderate < Severe)
cluster_means <- aggregate(Latent_Severity ~ GMM_Cluster, data = patients_complete, mean)
cluster_order <- order(cluster_means$Latent_Severity)
label_map <- c("Mild", "Moderate", "Severe")[1:length(cluster_order)]

patients_complete$Severity_State <- factor(patients_complete$GMM_Cluster, 
                                            levels = cluster_order, 
                                            labels = label_map)

# Visualize GMM Density Clusters
ggplot(patients_complete, aes(x = Latent_Severity, fill = Severity_State)) +
  geom_density(alpha = 0.5) +
  theme_bw() +
  labs(title = "Stage 3: Natural Severity States via GMM", 
       x = "Latent Severity Score", y = "Density") +
  scale_fill_manual(values = c("Mild" = "green3", "Moderate" = "orange", "Severe" = "red3"))

## Stage 4: Integrate Transcriptomic Structure
# Perform PCA on batch-corrected RNA-seq data to capture major molecular gradients
common_samples <- intersect(rownames(patients_complete), colnames(expr_rb))
expr_complete <- expr_rb[, common_samples]

pca_rna <- prcomp(t(expr_complete))
patients_complete$RNA_PC1 <- pca_rna$x[common_samples, 1]
patients_complete$RNA_PC2 <- pca_rna$x[common_samples, 2]

# Dual-axis visualization: Clinical vs Transcriptomic
ggplot(patients_complete, aes(x = Latent_Severity, y = RNA_PC1, color = Severity_State)) +
  geom_point(size = 4, alpha = 0.8) +
  theme_bw() +
  labs(title = "Stage 4: Clinical-Transcriptomic Integration", 
       subtitle = "Aligning clinical latent axis with major RNA variance",
       x = "Latent Clinical Severity", y = "RNA PC1 (Molecular Staging)") +
  scale_color_manual(values = c("Mild" = "green3", "Moderate" = "orange", "Severe" = "red3"))

## Stage 5: Machine Learning Classifier for All Patients
# Train a classifier to predict severity for patients lacking complete clinical scores
set.seed(42)

# Features: Top 1000 variable genes from RNA-seq (to avoid overfitting)
top_var_genes <- order(rowVars(expr_complete), decreasing = TRUE)[1:1000]
train_data <- t(expr_complete[top_var_genes, ])
train_labels <- patients_complete$Severity_State

# Random Forest Model
rf_model <- randomForest(x = train_data, y = train_labels, ntree = 1000, importance = TRUE)

# Extend to all patients
all_patients_meta <- metadata_frda |>filter(Condition == "Patient")
expr_all_patients <- expr_rb[top_var_genes, rownames(all_patients_meta)]

pred_labels <- predict(rf_model, t(expr_all_patients))
pred_probs <- predict(rf_model, t(expr_all_patients), type = "prob")

all_patients_meta$Stratified_Severity <- pred_labels
all_patients_meta <- cbind(all_patients_meta, as.data.frame(pred_probs))

# Save the final results
dir.create("patient_stratification_prediction", showWarnings = FALSE)
saveRDS(all_patients_meta, "patient_stratification_prediction/final_severity_stratification.rds")
write.xlsx(all_patients_meta, "patient_stratification_prediction/final_severity_stratification.xlsx", rowNames = TRUE)

## Stage 6 & 7: Validation and Biological Insights
# 1. Clinical Validation: Monotonic increase of mFARS across predicted groups
p_clin <- ggplot(all_patients_meta, aes(x = Stratified_Severity, y = `mFARS total`, fill = Stratified_Severity)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7) +
  geom_jitter(width = 0.2, alpha = 0.5) +
  stat_compare_means(comparisons = list(c("Mild", "Moderate"), c("Moderate", "Severe"), c("Mild", "Severe"))) +
  theme_bw() +
  scale_fill_manual(values = c("Mild" = "green3", "Moderate" = "orange", "Severe" = "red3")) +
  labs(title = "Clinical Validation", y = "mFARS Total Score", x = "Predicted Severity")

# 2. Biological Validation: FXN Expression Gradient
# Merging with previously calculated FXN expression data
fxn_val_df <- all_patients_meta |>
  rownames_to_column("SampleID") |>
  left_join(data_fxn |>select(SampleID, FXN_expression), by = "SampleID")

p_bio <- ggplot(fxn_val_df, aes(x = Stratified_Severity, y = FXN_expression, fill = Stratified_Severity)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7) +
  geom_jitter(width = 0.2, alpha = 0.5) +
  theme_bw() +
  scale_fill_manual(values = c("Mild" = "green3", "Moderate" = "orange", "Severe" = "red3")) +
  labs(title = "Biological Validation", y = "FXN Expression (VST)", x = "Predicted Severity")

(p_clin | p_bio)

ggsave("patient_stratification_prediction/validation_summary.png", width = 14, height = 6)
```

# Correlation of gene expression profiles with severity and decoding patient trajectories
```{r}
dds <- readRDS("dds_frda_all_ensg_removed.rds")
dds_patient <- dds[, (dds$Condition == "Patient") & (dds$GAA1 > dds$GAA1[dds$Condition == "Control"][1]) & (!is.na(dds$GAA1)) & (!is.na(dds$DD)) & (!is.na(dds$`FSA scale`)) & (!is.na(dds$`mFARS total`)) & (!is.na(dds$`mFARS USS`))]
meta <- as.data.frame(colData(dds_patient))
counts <- as.data.frame(assay(dds_patient))
traits_of_interest <- c("FSA.scale", "mFARS.total", "mFARS.USS", "GAA1", "DD")
for (trait in traits_of_interest) {
    meta[[trait]] <- as.numeric(as.character(meta[[trait]]))
}

vsd <- vst(dds_patient, blind = TRUE)
expr_vst <- assay(vsd)

msig_hallmark <- msigdbr(species = "Homo sapiens", collection = "H")

gene_sets <- split(msig_hallmark$gene_symbol,
                   msig_hallmark$gs_name)

# Linear regression and non-linear trends for each gene with each trait
all_linear_results  <- list()
all_spline_results  <- list()
all_camera_results  <- list()

# Question: Is gene expression significantly associated with increasing severity score?
for (trait in traits_of_interest) {

  cat("\nProcessing trait:", trait, "\n")

  clean_samples <- complete.cases(meta[, trait])
  meta_sub   <- meta[clean_samples, ]
  counts_sub <- counts[, clean_samples]

  # Center trait
  meta_sub[[trait]] <- scale(meta_sub[[trait]], center = TRUE, scale = FALSE)

  # Build DGE
  dge <- DGEList(counts = counts_sub)
  cpm_vals <- cpm(dge)
  keep <- rowSums(cpm_vals > 1) >= 10
  dge  <- dge[keep, , keep.lib.sizes = FALSE]
  dge  <- calcNormFactors(dge)

  # Linear Model
  design_linear <- model.matrix(
    as.formula(paste0("~ `", trait, "`")),
    data = meta_sub
  )

  v_linear <- voom(dge, design_linear)

  fit_linear <- lmFit(v_linear, design_linear)
  fit_linear <- eBayes(fit_linear, robust = TRUE)
  ...
  all_linear_results[[trait]] <- res_linear

  # Spline Model
  design_spline <- model.matrix(
    as.formula(paste0("~ ns(`", trait, "`, df=3)")),
    data = meta_sub
  )

  v_spline <- voom(dge, design_spline)

  fit_spline <- lmFit(v_spline, design_spline)
  fit_spline <- eBayes(fit_spline, robust = TRUE)

  spline_terms <- grep("^ns", colnames(design_spline), value = TRUE)
  ...
  all_spline_results[[trait]] <- res_spline

  # Pathway Analysis (camera)
  camera_res <- camera(
    v_linear,
    index = gene_sets,
    design = design_linear,
    contrast = trait
  )

  camera_res <- as.data.frame(camera_res)
  camera_res$pathway <- rownames(camera_res)

  all_camera_results[[trait]] <- camera_res

  cat("Significant genes with linear regression (p-value<0.05):",
      sum(res_linear$P.Value < 0.05), "\n")

  cat("Significant genes with linear regression (FDR<0.05):",
      sum(res_linear$adj.P.Val < 0.05), "\n")

  cat("Significant genes with splines (p-value<0.05):",
      sum(res_spline$P.Value < 0.05), "\n")
      
  cat("Significant genes with splines (FDR<0.05):",
      sum(res_spline$adj.P.Val < 0.05), "\n")

  cat("Significant pathways (FDR<0.05):",
      sum(camera_res$FDR < 0.05), "\n")
}

# WGCNA Module-Trait Correlation (bicor)
enableWGCNAThreads()
wgcna_res <- readRDS("wgcna/FRDA_All_Batches_ENSG_genes_removed/wgcna_frda_all_batch.rds")
MEs <- wgcna_res$MEs

MEs_patient <- MEs[rownames(meta), ]

# Calculate bicor (Biweight midcorrelation)
module_trait_cor <- bicor(MEs_patient, meta[, traits_of_interest], use = "p")
module_trait_p <- corPvalueStudent(module_trait_cor, nrow(MEs_patient))

wgcna_cor_df <- as.data.frame(module_trait_cor) |>rownames_to_column("Module")
wgcna_p_df <- as.data.frame(module_trait_p) |>rownames_to_column("Module")

write_xlsx(
    c(all_linear_results, list(WGCNA_Cor = wgcna_cor_df, WGCNA_P = wgcna_p_df)),
    "patient_stratification_prediction/patient_stratify/regression_severity_scores.xlsx"
)

write_xlsx(
  all_spline_results, "patient_stratification_prediction/patient_stratify/spline_severity_scores.xlsx"
)

write_xlsx(
  all_camera_results, "patient_stratification_prediction/patient_stratify/pathways_severity_scores.xlsx"
)

# Top genes from all correlations
all_res_linear <- lapply(traits_of_interest, function(tr) {
  read_excel("patient_stratification_prediction/patient_stratify/regression_severity_scores.xlsx", sheet = tr) |>
    mutate(Trait = tr) |>
    select(gene, logFC, P.Value, adj.P.Val, Trait)
})

all_res_non_linear <- lapply(traits_of_interest, function(tr) {
  read_excel("patient_stratification_prediction/patient_stratify/spline_severity_scores.xlsx", sheet = tr) |>
    mutate(Trait = tr) |>
    select(gene, P.Value, adj.P.Val, Trait)
})

top_table_linear <- bind_rows(all_res_linear) |>
  group_by(Trait) |>
  arrange(P.Value, .by_group = TRUE) |>
  slice_head(n = 5) |>
  ungroup()

top_table_non_linear <- bind_rows(all_res_non_linear) |>
  group_by(Trait) |>
  arrange(P.Value, .by_group = TRUE) |>
  slice_head(n = 5) |>
  ungroup()

top_genes_linear <- top_table_linear |>
  pull(gene) |>
  unique()

top_genes_non_linear <- top_table_non_linear |>
  pull(gene) |>
  unique()

final_genes <- c(top_genes_linear, top_genes_non_linear) |> unique()

# Plots
# Trend of gene expression with severity
# Extract expression data for top genes
plot_trends <- function(trait) {

    # Extract expression data for top genes
    plot_data_list <- list()
    for (gene in final_genes) {
        if (gene %in% rownames(expr_vst)) {
            df <- data.frame(
                Score = meta[[trait]],
                Expression = expr_vst[gene, ],
                Gene = gene
            ) |> drop_na()
    ...
    p <- ggplot(all_plot_data, aes(x = Score, y = Expression)) +
        geom_point(alpha = 0.5, color = "#2166ac") +
        geom_smooth(method = "lm", color = "#b2182b", fill = "#f4a582", alpha = 0.2) +
        facet_wrap(~Gene, scales = "free_y") +
        theme_bw() +
        labs(
            title = paste("Expression Trends vs", trait),
            x = trait,
...
p1 <- plot_trends("FSA.scale")
p2 <- plot_trends("mFARS.total")
p3 <- plot_trends("GAA1")

ggsave("patient_stratification_prediction/patient_stratify/expression_trends_FSA.png", p1, width = 12, height = 8, dpi = 300)
ggsave("patient_stratification_prediction/patient_stratify/expression_trends_mFARS.png", p2, width = 12, height = 8, dpi = 300)
ggsave("patient_stratification_prediction/patient_stratify/expression_trends_GAA1.png", p3, width = 12, height = 8, dpi = 300)

# Volcano plots for each trait
plot_volcano_cor <- function(trait_name) {
  df <- read_excel("patient_stratification_prediction/patient_stratify/regression_severity_scores.xlsx", sheet = trait_name)
  ...
  top_n <- 15
  top_genes <- df |> arrange(P.Value) |> head(top_n)
  ...
  ggplot(df, aes(x = logFC, y = negLogP)) +
    ...
    theme_minimal() +
    labs(title = paste("Association with", trait_name),
         subtitle = "Robust linear regression (limma)",
         x = "Coefficient (Expression change per unit trait)",
         y = "-log10(p-value)") +
    theme(legend.position = "none")

    ggsave(paste0("patient_stratification_prediction/patient_stratify/volcano_plots_", trait_name, ".png"), width = 10, height = 8)
}

volcano_plots <- lapply(traits_of_interest, plot_volcano_cor)

# Heatmap of WGCNA module-trait correlations
cor_mat <- as.matrix(wgcna_cor_df[,-1])
rownames(cor_mat) <- wgcna_cor_df$Module
p_mat <- as.matrix(wgcna_p_df[,-1])
rownames(p_mat) <- wgcna_p_df$Module

# Signif stars
text_mat <- matrix(ifelse(p_mat < 0.001, "***", 
                          ifelse(p_mat < 0.01, "**", 
                                 ifelse(p_mat < 0.05, "*", ""))), 
                   nrow = nrow(p_mat))
...
pdf("patient_stratification_prediction/patient_stratify/wgcna_module_trait_correlation.pdf", width = 10, height = 8)
Heatmap(cor_mat, 
        name = "Bicor", 
        col = col_fun,
        cluster_rows = TRUE, 
        ...
        cell_fun = function(j, i, x, y, width, height, fill) {
          grid.text(text_mat[i, j], x, y, gp = gpar(fontsize = 15, fontface = "bold"))
        },
        column_title = "WGCNA Module - Clinical Trait Correlation (bicor)",
        rect_gp = gpar(col = "white", lwd = 1))
dev.off()

# Molecular severity score via PCA on top 500 variable genes
gene_variances <- apply(expr_vst, 1, var)
top500_genes <- names(sort(gene_variances, decreasing = TRUE))[1:500]
expr_top500 <- t(expr_vst[top500_genes, ])

pca <- prcomp(expr_top500, scale. = TRUE)

meta_sub$Molecular_Severity_PC1 <- pca$x[,1]

for (trait in traits_of_interest) {
  cor_val <- cor(meta_sub$Molecular_Severity_PC1,
                 meta_sub[[trait]],
                 use="complete.obs")

  cat("Correlation between PC1 and", trait, ":", cor_val, "\n")
}

write.csv(meta_sub,
          "patient_stratification_prediction/patient_stratify/FRDA_molecular_severity_scores.csv",
          row.names = FALSE)
```
